 ...
 * Ingests the fixture corpus (idempotent), runs the golden evaluation suite, compares with the
 * baseline, writes the HTML QA report, and exits non-zero on regression or unmet success targets.
 */
async function main() {
  ...
  const nCases = await seedEvalCases();
  log(`eval cases: ${nCases}`);
  ...
  const outcome = await runEvaluation({
    ...
    userId: seed.adminId,
    processingRunId: corpusRun?.id ?? null,
    setBaseline: args.has("--set-baseline"),
    reuseCachedAnswers: !args.has("--no-cache"),
    log,
  });
  const m = outcome.metrics;
  const rows: [string, number, number][] = [
    ["scalar exact accuracy", m.extraction.scalar_exact_accuracy, SUCCESS_TARGETS.scalarExactAccuracy],
    ["list micro-F1", m.extraction.list_micro_f1, SUCCESS_TARGETS.listMicroF1],
    ["evidence validity", m.extraction.evidence_validity, SUCCESS_TARGETS.evidenceValidity],
    ...
    ["retrieval recall@5", m.rag.retrieval_recall_at_5, SUCCESS_TARGETS.retrievalRecallAt5],
    ["citation precision", m.rag.citation_precision, SUCCESS_TARGETS.citationPrecision],
    ["evidence-supported answers", m.rag.evidence_supported_rate, SUCCESS_TARGETS.evidenceSupportedRate],
    ["refusal accuracy", m.rag.refusal_accuracy, SUCCESS_TARGETS.refusalAccuracy],
    ["semantic score", m.rag.semantic_score, SUCCESS_TARGETS.semanticScore],
  ];
  ...
  console.table(outcome.regression.checks.map((c) => ({ metric: c.metric, baseline: c.baseline === null ? "n/a" : c.baseline.toFixed(3), current: c.current.toFixed(3), rule: c.threshold, ok: c.passed ? "yes" : "NO" })));
  log(`integrity: duplicates ${m.integrity.duplicate_cases_passed}/${m.integrity.duplicate_cases}, versions ${m.integrity.version_cases_passed}/${m.integrity.version_cases}, duplicate records ${m.integrity.duplicate_records}, missing review items ${m.review.below_threshold_missing}`);
  ...
  const ok = outcome.regression.passed && (args.has("--no-strict") || targetsOk);
  await closeDb();
  if (!ok) {
